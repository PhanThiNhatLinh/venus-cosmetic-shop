...
<div class="container-fluid p-0 mb-5 pb-5">
  <img style="height: 600px" class="w-100" src="{{asset('/admin/images/brand/'.$item['thumb'])}}" alt="{{$item['name']}}">
</div>
...
<div class="container-fluid py-5">
  <div class="container py-5">
    <div class="row justify-content-center">
        <div class="col-lg-7">
            <h1 class="section-title position-relative text-center mb-5">Những sản phẩm thuộc thương hiệu <span style="color:green ">{{strtoupper($item['name'])}}</span></h1>
        </div>
    </div>
    <div class="row" >
          ...
          @foreach ($thumbs as $thumb)
            @if(!empty($thumb))
              <div class="position-relative bg-primary rounded-circle mt-n3 mb-4 p-3" style="width: 255px; height: 255px;" bis_skin_checked="1">
                <a href="{{$link}}"><img class="rounded-circle w-100 h-100" src="{{asset('/admin/images/product/'.$thumb)}}" style="object-fit: cover;"></a>
              </div>
              @break;
            @endif
          @endforeach    
          <a href="{{$link}}"><h5 class="font-weight-bold mb-4">{{$name}}</h5></a>
          @if($product['stock']>0)
