                        ...
                        <div class="form-group"> 
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                @foreach ($permissions as $k=> $permission)
                                    @if(in_array($permission['id'],$role_permissions_id))
                                        <input type="checkbox" checked id="{{$permission['id']}}" name="permission_checkbox[]" value="{{$permission['id']}}">
                                        <label for="{{$permission['id']}}"> {{$permission['name']}}</label><br>
                                    @else
                                        <input type="checkbox" id="{{$permission['id']}}" name="permission_checkbox[]" value="{{$permission['id']}}">
                                        <label for="{{$permission['id']}}"> {{$permission['name']}}</label><br>
                                    @endif
                                @endforeach
                            </div>
