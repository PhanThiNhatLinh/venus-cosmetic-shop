...
class Category extends Model
{
    ...
    protected $image_path='/admin/images/category';
    ...
    public function getListItems($params=null,$options = null){
        ...
        if($options['task'] == 'admin_get_list_items'){
            ...
            $query = self::select('id','name','status','display','thumb','created_by','modified_by');
        ...
        if($options['task'] == 'frontend_get_list_items'){
            $query = self::select('id','name','status','display','thumb');
            $query->where('status','active')->where('display','yes'); 
            $results = $query->take(6)->get()->toArray();
        }
        return $results;
    }
